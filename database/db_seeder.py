...
def seed_database():
    ...
    # 4. Seed 105 Realistic UMaT Listings Procedurally
    ...
    # Generate 105Unique Seed Listings procedurally from templates
    listings = []
    ...
    for cat_id, tmpl_list in TEMPLATES.items():
        for item in tmpl_list:
            ...
            listing_count += 1
            # Use the semantic product image path from the template (item[10])
    ...
    # Fill remaining count up to 60 listings procedurally with variant names (reusing real product photos)
    var_idx = 1
    while len(listings) < 60:
        cat_id = (var_idx % 13) + 1
        ...
        # Variant units reuse the exact same real product photo as the base product
    ...
    # 5. Seed Transactions, Requests, Reviews, Wishlist, Saved Listings
